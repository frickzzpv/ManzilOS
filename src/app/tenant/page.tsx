...
export default function TenantDashboard() {
  ...
  const { data: leaseInfo, isLoading: isLeaseLoading, refetch: refetchLeaseInfo } = useLeaseInfo()
  ...
  const handleRenewalResponse = async (status: 'ACCEPTED' | 'DECLINED') => {
      toast.info(`Responding to renewal offer with: ${status}`);
      // In a real app, this would call an API endpoint
      // For now, we just simulate it and refetch the lease info
      await new Promise(res => setTimeout(res, 1000));
      await api.post(`/api/tenant/lease/renewal`, { status });
      refetchLeaseInfo();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        ...
        {leaseInfo?.renewalOfferStatus === 'SENT' && (
            <Card className="mb-8 bg-blue-50 border-blue-200">
                <CardHeader>
                    <CardTitle>Lease Renewal Offer</CardTitle>
                    <CardDescription>Your landlord has sent you an offer to renew your lease.</CardDescription>
                </CardHeader>
                <CardContent className="flex items-center space-x-4">
                    <Button onClick={() => handleRenewalResponse('ACCEPTED')}>Review and Accept</Button>
                    <Button variant="ghost" onClick={() => handleRenewalResponse('DECLINED')}>Decline</Button>
                </CardContent>
            </Card>
        )}
